...
EditorToolPalette::EditorToolPalette(wxWindow* parent, const wxPoint& pos, const wxSize& size,
            long style):
  BaseClass(parent, wxID_ANY, pos, size, style)
{
    currentlySelected = -1;
}
...
{
  BaseClass::SetSize(x, y, width, height, sizeFlags);
}
...
EditorToolPalette *MyApp::CreatePalette(wxFrame *parent)
{
  ...
  palette->AddTool(PALETTE_ARROW, _T("Pointer"), PaletteArrow, "Select", wxITEM_RADIO);
  palette->AddTool(PALETTE_TOOL1, _T("Tool 1"), PaletteTool1, "Rectangle", wxITEM_RADIO);
  palette->AddTool(PALETTE_TOOL2, _T("Tool 2"), PaletteTool2, "Rounded Rectangle", wxITEM_RADIO);
  palette->AddTool(PALETTE_TOOL3, _T("Tool 3"), PaletteTool3, "Circle", wxITEM_RADIO);
  palette->AddTool(PALETTE_TOOL4, _T("Tool 4"),  PaletteTool4, "Diamon", wxITEM_RADIO);
