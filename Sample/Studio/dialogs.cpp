...
csLabelEditingDialog::csLabelEditingDialog(wxWindow* parent)
{
    ...
    wxXmlResource::Get()->LoadDialog(this, parent, _T("shape_label_dialog"));
    ...
    wxDialog(parent, wxID_ANY, _T("Settings"), wxPoint(0, 0), wxSize(PROPERTY_DIALOG_WIDTH, PROPERTY_DIALOG_HEIGHT))
{
    ...
    m_generalSettings = wxXmlResource::Get()->LoadPanel(m_notebook, _T("general_settings_dialog"));
    wxASSERT_MSG( (m_generalSettings), _T("Could not load general settings panel."));
    ...
    m_diagramSettings = wxXmlResource::Get()->LoadPanel(m_notebook, _T("diagram_settings_dialog"));
    wxASSERT_MSG( (m_diagramSettings), _T("Could not load diagram settings panel."));
    ...
    wxDialog(parent, wxID_ANY, title, wxPoint(0, 0), wxSize(SHAPE_PROPERTY_DIALOG_WIDTH, SHAPE_PROPERTY_DIALOG_HEIGHT))
{
    ...
    bool success = wxXmlResource::Get()->LoadPanel(m_generalPropertiesDialog, m_notebook, _T("general_shape_properties_dialog"));
    wxASSERT_MSG( (m_generalPropertiesDialog), _T("Could not load general properties panel."));
    m_notebook->AddPage(m_generalPropertiesDialog, _T("General"));

    success = wxXmlResource::Get()->LoadPanel(m_attributeDialog, m_notebook, attributeDialogName);
    if (!success) // wxLoadFromResource(m_attributeDialog, m_notebook, attributeDialogName)
    {
    ...
    str += _T("1");
    m_alternativeAttributeDialog = wxXmlResource::Get()->LoadPanel(m_notebook, str);
