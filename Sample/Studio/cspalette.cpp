...
bool csApp::CreatePalette(wxFrame *parent)
{
  ...
  palette->AddTool(PALETTE_ARROW, _T("Pointer"), PaletteArrow, wxEmptyString, wxITEM_RADIO);
  palette->AddTool(PALETTE_TEXT_TOOL, _T("Text"), TextTool, wxEmptyString, wxITEM_RADIO);

  wxChar ** symbols = new wxChar*[20];
  int noSymbols = 0;

  symbols[noSymbols] = (wxChar*)_T("Wide Rectangle");
  noSymbols ++;

  symbols[noSymbols] = (wxChar*)_T("Thin Rectangle");
  noSymbols ++;

  symbols[noSymbols] = (wxChar*)_T("Triangle");
  noSymbols ++;

  symbols[noSymbols] = (wxChar*)_T("Octagon");
  noSymbols ++;

  // For some reason, we're getting Gdk errors with
  // some shapes, such as ones that use DrawEllipse.
#ifndef __WXGTK__
  symbols[noSymbols] = (wxChar*)_T("Group");
  noSymbols ++;

  symbols[noSymbols] = (wxChar*)_T("Circle");
  noSymbols ++;

  symbols[noSymbols] = (wxChar*)_T("Circle shadow");
  noSymbols ++;

  symbols[noSymbols] = (wxChar*)_T("SemiCircle");
  noSymbols ++;
#endif
  ...
  for (i = 0; i < noSymbols; i++)
  {
      ...
      if (symbol)
      {
           ...
           palette->AddTool(symbol->GetToolId(), symbol->GetName(), *bitmap, wxEmptyString, wxITEM_RADIO);
  ...
  while (node)
  {
    ...
    wxBitmap* bitmap = GetSymbolDatabase()->CreateToolBitmap(symbol, toolBitmapSize);
    palette->AddTool(symbol->GetToolId(), symbol->GetName(), *bitmap);
