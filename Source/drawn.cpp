...
// Modified by:
// Created:     12/07/98
// RCS-ID:      $Id: drawn.cpp,v 1.1 2007/03/28 15:15:57 frm Exp $
...
void wxOpSetGDI::Do(wxDC& dc, double WXUNUSED(xoffset), double WXUNUSED(yoffset))
{
  switch (m_op)
  {
    case DRAWOP_SET_PEN:
    {
      ...
      // colour
      if (m_image->m_outlineColours.Member((wxObject*)(uintptr_t)m_gdiIndex))
      {
    ...
    case DRAWOP_SET_BRUSH:
    {
      ...
      else if (m_image->m_fillColours.Member((wxObject*)(uintptr_t)m_gdiIndex))
      {
...
void wxPseudoMetaFile::ReadAttributes(wxExpr *clause, int whichAngle)
{
    ...
    else
    {
      ...
      switch (idExpr->IntegerValue())
      {
        case gyTYPE_PEN:
        {
          ...
          wxPen *p = wxThePenList->FindOrCreatePen(col, penWidth, (wxPenStyle)penStyle);
          if (!p)
            p = wxBLACK_PEN;
        ...
        case gyTYPE_BRUSH:
        {
          ...
          wxBrush *b = wxTheBrushList->FindOrCreateBrush(col, (wxBrushStyle)brushStyle);
          if (!b)
            b = wxWHITE_BRUSH;
        ...
        case gyTYPE_FONT:
        {
          ...
          m_gdiObjects.Append(wxTheFontList->FindOrCreateFont(fontPointSize,
                           fontFamily, (wxFontStyle)fontStyle, fontWeight, (fontUnderlined != 0)));
          break;
        }
        default:
        {
  ...
  if (expr1)
  {
    ...
    while (eachExpr)
    {
      m_fillColours.Append(eachExpr->IntegerValue());
      eachExpr = eachExpr->GetNext();
    }
  }
}
...
void wxPseudoMetaFile::Copy(wxPseudoMetaFile& copy)
{
  ...
  // Copy the outline/fill operations
    ...
    while (node)
    {
      copy.m_outlineColours.Append(node->GetData());
      node = node->GetNext();
    }
    node = m_fillColours.GetFirst();
    while (node)
    {
      copy.m_fillColours.Append(node->GetData());
      node = node->GetNext();
    }
}
...
void wxPseudoMetaFile::SetPen(const wxPen* pen, bool isOutline)
{
    ...
    if (isOutline)
    {
        m_outlineColours.Append((wxObject*)(uintptr_t)(n - 1));
    }
}
...
void wxPseudoMetaFile::SetBrush(const wxBrush* brush, bool isFill)
{
    ...
    if (isFill)
    {
        m_fillColours.Append((wxObject*)(uintptr_t)(n - 1));
    }
}
